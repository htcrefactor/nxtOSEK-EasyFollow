...
#define RUNTIME_CONNECTION
  ...
  int Speed_H=75;// 설정된 고속
  int Speed_L=55;// 설정된 저속
...
//조향장치
// 두 초음파 센서의 값이 4이상 차이나면 조향 장치를 변화시킨다.
// 초음파 센서의 값 변화가 예측 불가능해서 다양한 범위로 나누어서 조향장치를 제어했다.
// 먼저 두 센서 모두 23이하일 경우 가까운 경우로 생각해서 차이가 4-6 사이의 경우만 조향장치가 작동하도록 했다.
// 이는 조향장치가 확실하게 작동해야 하는 범위로 가까운 거리에서 두 센서 모두 값이 정확할 때를 고려해서 해당 범위를 만들었다.
//하지만 최종적으로 위의 범위를 아래 if문이 모두 포함한다는 것을 확인하고 범위를 나누어서 테스트했지만 이는 성공적이지는 못했다.
/*
if(Actual_Sonar_Num_1-Actual_Sonar_Num_2> 3 && Actual_Sonar_Num_1-Actual_Sonar_Num_2< 7 && Actual_Sonar_Num_2<=23 && Actual_Sonar_Num_1<=23)
{
  bt_receive_buf[4] = 3;
}
else if(Actual_Sonar_Num_2-Actual_Sonar_Num_1> 3 && Actual_Sonar_Num_2-Actual_Sonar_Num_1< 7 && Actual_Sonar_Num_2<=23 && Actual_Sonar_Num_1<=23)
{
  bt_receive_buf[4] = 4;
}
*/
//만약 한쪽 초음파 센서만 값이 있을경우 그쪽 방향으로 조향 장치를 이동한다.
// 한쪽 센서의 값이 존재하지 않을 때 조향장치를 작동시켰다. 
// 가까운 거리에서는 판과 초음파 센서의 각도가 커져도 3정도 밖에 차이가 나지 않고 이 경우에는 조향장치가 작동하지 않게 했다. 
// 가까운 거리에서 조향장치가 자주 작동하면 지속적으로 방향을 바꾸게 되고 주행 능력이 떨어진다는 것을 확인했다. 
if(Actual_Sonar_Num_1-Actual_Sonar_Num_2> 3||(Actual_Sonar_Num_1==100&&Actual_Sonar_Num_2<50) )
{
  bt_receive_buf[4] = 3;
}
else if(Actual_Sonar_Num_2-Actual_Sonar_Num_1> 3 ||(Actual_Sonar_Num_2==100&&Actual_Sonar_Num_1<50) )
{
  bt_receive_buf[4] = 4;
}
else if(Actual_Sonar_Num_2< 50 && Actual_Sonar_Num_1<50)
{bt_receive_buf[4] = 0;}
//else가 없는 이유는 한쪽 방향으로 이동중에는 앞차의 회전에 의해서 두 초음파
//센서의 값 모두 못 받을 수 있어서 계속 그 방향으로 이동시키기 위해



// 속도 조절
// 두 초음파 센서의 값 모두 존재할 경우 모터를 작동시킨다
if(Actual_Sonar_Num_1<50 && Actual_Sonar_Num_2<50)
{
//거리가 멀지 않고 적당히 떨어져 있으면 저속으로 주행
if((Actual_Sonar_Num_1+Actual_Sonar_Num_2)/2<26 &&(Actual_Sonar_Num_1+Actual_Sonar_Num_2)/2>20)
{
    Current_speed=Speed_L;
    Motor_Run=1;
    bt_receive_buf[3]=1;
...
//멀리 떨어져 있으면 고속으로 주행
else if((Actual_Sonar_Num_1+Actual_Sonar_Num_2)/2>=26)
{
...
//한쪽 센서의 값만 존재하는 경우 회전하는 경우로 인식하고 속도는 저속으로 한다.
else if(Actual_Sonar_Num_1<50 || Actual_Sonar_Num_2<50)
{
    Current_speed=Speed_L;
    Motor_Run=1;
    bt_receive_buf[3]=1;
...
//두 센서에 어떠한 값도 오지 않을 경우 연속적인 700카운트를 확인하고 700에
// 도달하면 정지하고 소리를 발생한다.
else
{
   ++Terminate_Check;
...
// 이전에 이동한 방향의 반대로 이동하게 되고 원래 위치로 이동하게 된다. 
  ...
  if (err > 0)
  {
    nxt_motor_set_speed(NXT_PORT_A, (err/2 + 10),1);
  }
  else if (err < 0)
  {
    nxt_motor_set_speed(NXT_PORT_A, (err/2 - 10),1);
...
TASK(SpeedTask)
{
  ...
  ClearEvent(event1);
  GetResource(R1);
  if(Current_speed==Speed_L) {Current_speed=Speed_H;}
  else if(Current_speed==Speed_H) {Current_speed=Speed_L;}
...
TASK(BrakeTask)
{
  ...
  while(1)
  {
    ...
    {
    if(Current_speed==Speed_H && Motor_Run==1) {Slow_brake=Speed_H*2;}
    else if(Current_speed==Speed_L && Motor_Run==1) {Slow_brake=Speed_L*2;}
    Motor_Run=0;
    }
    ReleaseResource(R1);
  }
